...
> 历史压缩目标：**下线 32 个旧名、增加 8 个聚合入口，193 → 169 工具（净减 24，-12%）**，**零功能缺失**
> 度量口径: 以 Conda 环境中完整序列化的 `tools/list` 字符数为准；token 数不再用“字符数 ÷ 4”代替真实 tokenizer 结果
> 历史实施结果（Wave 4）：**14 server / 169 工具 / 完整 `tools/list` 141,035 字符**；v2 manifest 为 32 条唯一路由。
> 当前实时拓扑：14 server / 180 工具 / 242 Resource，工具 `outputSchema` 覆盖率 180/180。
...
4. 历史过渡期工具数为 **201（193 + 8）**，公开面会暂时增加；当时摘除 32 个旧名后达到 169。
...
**历史稳定态工具数为 193 → 169（-12%）**。完整公开面由 160,630 降至当时实测 141,035 字符（-12.2%）；该值来自实际聚合 schema 的完整序列化，不用 schema-only 字符或“字符数 ÷ 4”推导 token 结论。当前实时拓扑见本文开头快照。
...
1. **是否启用过渡期**：本历史修订建议启用。新旧名并存时工具数会从 193 增至 201，公开面暂时增加；当时摘除 32 个旧名后降至 169。
...
3. `tests/fixtures/baseline/tools-list/*.json` 只在每个阶段代码与工具数稳定后重生成；历史过渡期 201 与稳定态 169 分别保留可审计快照。当前实时分母为 180 工具 / 242 Resource。
