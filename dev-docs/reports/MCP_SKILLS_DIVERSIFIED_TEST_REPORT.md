...
> 历史快照：本文记录 Wave 2/Wave 4 时期的测试分母（24/262 或 14/169）。这些数字仅代表当时的运行时，
> 不代表当前能力。当前实时拓扑（2026-08-30）为 14 server / 180 tools / 242 Resource，`outputSchema` 覆盖率 180/180。
...
**状态**: 历史文档（Wave 2 拓扑） — 本轮测试针对当时的 24 服务、262 工具；Wave 4 历史快照为 14 服务、169 工具。当前实时拓扑为 14 server / 180 tools / 242 Resource，工具 `outputSchema` 覆盖率 180/180。
