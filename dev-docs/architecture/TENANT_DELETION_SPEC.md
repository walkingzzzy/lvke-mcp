...
1. **独立化**（[`MCP_INDEPENDENCE_PLAN.md`](MCP_INDEPENDENCE_PLAN.md)）：脱离 `hermes_cli` 依赖，独立运行、独立测试。
...
## 8. 与 [`MCP_INDEPENDENCE_PLAN.md`](MCP_INDEPENDENCE_PLAN.md) 的关系

- 本说明只负责「删安全门禁 + 删 tenant 维度」，**不解决** `hermes_cli` 的独立化依赖。
- 独立化仍按 [`MCP_INDEPENDENCE_PLAN.md`](MCP_INDEPENDENCE_PLAN.md) 进行（把纯计算层 lift-and-shift 搬移，不重写）。
