...
def _write_delivery_tables(wb, fin, Font, PatternFill, Alignment, Border, Side):
    ...
    for key, sheet_name in _DELIVERY_SHEETS.items():
        ...
        review_grade = fin.get("assurance_level") == "review_grade" and fin.get("review_status") == "approved"
        status_text = "评审级｜已批准" if review_grade else "匡算预览｜未批准，不得用于正式报批"
        ws.cell(2, 1, f"{status_text}；口径：{table.get('grade', 'summary')}；有效：{'是' if table.get('effective') else '否'}")
...
def export_finance_workbook(
    ...
    *,
    ...
    meta_rows = [
        ...
        ("formal_delivery_ready", bool(delivery_quality.get("formal_delivery_ready"))),
    ]
    ...
    formal_requested = bool(delivery_quality.get("formal_delivery_ready"))
    recalculation = (
    ...
    if formal_requested and not recalculation.get("ok"):
        ...
        delivery_quality["formal_delivery_ready"] = False
