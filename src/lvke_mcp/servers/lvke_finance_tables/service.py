...
def _load_run(
    ...
    return get_workspace_finance_run(
        workspace_id,
        run_id=run_id,
        view="full",
    )
...
def render(
    ...
    payload = {
        ...
        "formal_delivery_ready": bool(validation["formal_delivery_ready"]),
        "delivery_mode": "formal" if validation["formal_delivery_ready"] else "draft",
        "draft_only": not bool(validation["formal_delivery_ready"]),
...
def validate(
    ...
    *,
    ...
    rejected = _require_run_id(run_id)
    if rejected is not None:
        return {**rejected, **review_requirement}
    data = render_workspace_finance_tables(
        workspace_id,
        run_id=run_id,
        format="structured",
    ...
    result = _delivery_assessment(
        workspace_id,
        run_id,
        {
            **data,
            "tables": _structured_delivery_tables(
                workspace_id,
                run_id,
                data,
            ),
    ...
    business_success = technical_success if scope == "technical" else bool(
        technical_success and result["formal_delivery_ready"]
    )
    selected_blockers = (
    ...
    if not technical_success:
        next_actions = [
            "回到财务模型修正输入并生成新 run_id；表服务不得重算"
        ]
    elif scope == "formal" and not result["formal_delivery_ready"]:
        next_actions = [
            "完成财务 run 批准与工程门禁后，仍须执行统一交付物审查"
        ]
    elif scope == "technical":
        next_actions = ["技术结构校验通过；该结果不授予正式交付资格"]
    else:
        next_actions = ["工程校验已通过；仍须完成统一审查、签审与正式释放"]
    selected_validation = (
    ...
    return {
        ...
        "formal_delivery_ready": bool(result["formal_delivery_ready"]),
        "resource_uris": [],
...
def export_xlsx(
    ...
    formal_ready = bool(validation.get("formal_delivery_ready")) and bool(export_quality.get("formal_delivery_ready"))
    blockers = list(rendered.get("blockers") or [])
    if validation.get("formal_delivery_ready") and not export_quality.get("formal_delivery_ready"):
...
def export_csv(
    ...
    return {
        **rendered,
        ...
        "delivery_mode": "formal" if rendered.get("formal_delivery_ready") else "draft",
        "draft_only": not bool(rendered.get("formal_delivery_ready")),
        "resource_uris": [
...
def get_package(
    ...
    if not integrity["valid"]:
        ...
        result["formal_delivery_ready"] = False
...
def _formal_delivery_gate(
    ...
    """Assess deterministic table integrity without an authorization gate."""
    fail_closed = {"formal_delivery_ready": False, "approved_run_id": None}
    try:
    ...
    blockers = [
        str(item.get("code") or "finance_gate_blocker") if isinstance(item, dict) else str(item)
        for item in (result.get("blockers") or [])
    ]
    approved = str(result.get("approved_run_id") or "")
    ready = bool(result.get("ok")) and bool(approved) and approved == run_id
    if bool(result.get("ok")) and approved != run_id:
        blockers.append("approved_run_mismatch")
    if not ready and not blockers:
        blockers.append("finance_publish_gate_blocked")
    return {
        "formal_delivery_ready": ready,
        "blockers": blockers,
        "approved_run_id": approved or None,
    }
...
def _delivery_assessment(
    ...
    结构齐全（valid）只决定 package 状态 ok/partial；``formal_delivery_ready``
    必须结构齐全**且**宿主门禁全过，二者绝不互相冒充。
    """
    validation = _validate_render(data)
    run = _load_run(workspace_id, run_id)
    ...
    try:
        from lvke_mcp.servers.excel_bridge.finance_export import assess_finance_delivery_quality
        ...
        if semantic_blockers:
            validation["blockers"] = [
                *validation.get("blockers", []), *semantic_blockers,
            ]
            validation["valid"] = False
        ...
        validation["blockers"] = [
            *validation.get("blockers", []), "workbook_semantic_audit_failed",
        ]
        validation["valid"] = False
    ...
    gate = _formal_delivery_gate(workspace_id, run_id)
    boe_id = str(run.get("basis_of_estimate_id") or "")
    boe_hash = str(run.get("basis_of_estimate_hash") or "")
    boe_ready = False
    if boe_id and boe_hash:
        try:
            from lvke_mcp.servers.lvke_finance_model.server import BASIS_OF_ESTIMATE_STORE
            ...
            boe_ready = False
    if not boe_ready:
        gate["formal_delivery_ready"] = False
        gate["blockers"] = [*gate["blockers"], "basis_of_estimate_required"]
    formal_ready = bool(validation["valid"]) and bool(gate["formal_delivery_ready"])
    validation["formal_delivery_ready"] = formal_ready
    validation["assurance_level"] = "review_grade" if formal_ready else "estimate_preview"
    validation["gate_blockers"] = gate["blockers"]
    validation["approved_run_id"] = gate["approved_run_id"]
    ...
    # Technical validation verdict: ignores approval gate, only checks structural completeness
    validation["technical_validation_verdict"] = "pass" if validation["valid"] else "fail"
    if validation["valid"] and not formal_ready:
        validation["warnings"] = [*validation["warnings"], "宿主正式发布门禁未通过，禁止宣称正式交付"]
        validation["technical_validation_note"] = "13表结构完整、勾稽通过；正式交付需 finance_run 批准"
    return validation


def _validate_render(data: dict[str, Any]) -> dict[str, Any]:
    """Validate all thirteen tables down to columns, rows, scalar types and periods."""
    ...
    for key in delivery_keys:
        quality = _structured_table_quality(tables.get(key))
        table_quality[key] = quality
        if not quality["valid"]:
            blockers.extend(f"table_quality:{key}:{reason}" for reason in quality["blockers"])
        warnings.extend(f"{key}:{warning}" for warning in quality["warnings"])
    # The host model owns numerical finance tie-outs; surface its table-level
    # quality only if the renderer provided it, never invent a pass here.
    ...
    return {
        ...
        "blockers": blockers,
        "warnings": ([*warnings, "十三表结构、列级或期间校验未通过，禁止宣称正式交付"] if blockers else warnings),
        "table_quality": table_quality,
        "note": "表名齐全、CSV/XLSX 写出都不等于正式交付；正式资格仍由宿主发布门禁和结构化勾稽共同决定。",
    }
...
def _structured_delivery_tables(
    ...
    """Project immutable run data into row/column tables; no financial recomputation."""
    try:
        from lvke_mcp.domains.finance import table_render

        run = _load_run(workspace_id, run_id)
        structured = table_render.build_all_structured(run)
        structured.pop("_meta", None)
        if all(isinstance(structured.get(key), dict) for key in _delivery_keys()):
            return {key: structured[key] for key in _delivery_keys()}
    except Exception:
        pass
    # Legacy immutable runs persist markdown tables.  Parse only their GFM
    # projection as a last-resort read adapter; malformed tables fail quality.
    return {key: _markdown_table_as_structured((rendered.get("tables") or {}).get(key)) for key in _delivery_keys()}


def _markdown_table_as_structured(value: Any) -> dict[str, Any]:
    ...
    columns = [{"key": f"column_{index + 1}", "label": header} for index, header in enumerate(headers)]
    rows: list[list[Any]] = []
    for line in lines[2:]:
        cells = [cell.strip() for cell in line.strip("|").split("|")]
        if len(cells) != len(headers):
            continue
        rows.append([_parse_csv_scalar(cell) for cell in cells])
    return {"columns": columns, "column_labels": headers, "rows": rows, "row_count": len(rows), "source": "legacy_markdown_projection"}
...
def _structured_table_quality(table: Any) -> dict[str, Any]:
    """Verify renderer-owned structured shape without inferring missing finance facts."""
    blockers: list[str] = []
    warnings: list[str] = []
    if not isinstance(table, dict):
        return {"valid": False, "blockers": ["not_structured_table"], "warnings": [], "row_count": 0, "column_count": 0}
    columns = list(table.get("columns") or [])
    rows = list(table.get("rows") or [])
    keys = [str(column.get("key") or "") for column in columns if isinstance(column, dict)]
    labels = [str(column.get("label") or "") for column in columns if isinstance(column, dict)]
    ...
    # Period-oriented tables must expose a non-empty period key or period columns.
    period_indexes = [index for index, key in enumerate(keys) if key in {"year", "period"}]
    has_period_columns = any(key.startswith("period_") for key in keys)
    if period_indexes and any(not str(row[index]).strip() for row in rows if isinstance(row, list) and len(row) == len(columns) for index in period_indexes):
    ...
    return {"valid": not blockers, "blockers": blockers, "warnings": warnings, "row_count": len(rows), "column_count": len(columns), "nested_cell_count": nested, "duplicate_row_count": duplicate_rows}
...
def _package_result(record: dict[str, Any], validation: dict[str, Any], status: str) -> dict[str, Any]:
    ...
    return {
        ...
        "validation": validation,
        "formal_delivery_ready": bool(payload.get("formal_delivery_ready", False)),
