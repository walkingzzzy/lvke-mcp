...
TEMPLATES: dict[str, dict[str, Any]] = {
    ...
    # ── 9.4 审批进度 ────────────────────────────────────────────────
    "approval-schedule": {
        "template_id": "approval-schedule",
        "category": "approval-schedule",
        "name": "审批进度安排表",
        "description": "对应「研究结论及建议」的报批 / 决策建议。各专项审批必须早于开工时间。",
        "columns": [
            {"key": "seq", "label": "序号", "unit": "", "type": "string"},
            {"key": "task", "label": "审批事项", "unit": "", "type": "string"},
...
# 集中登记每张制式表的：
#   - appendix_no    附表序号（正文内表如风险矩阵/审批进度序号为空串）
...
_APPENDIX_META: dict[str, dict[str, Any]] = {
    ...
    "approval-schedule": {"appendix_no": "", "chapter_theme": "conclusion", "indicators": []},
