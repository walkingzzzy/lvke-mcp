...
def xlsx_path_from_uri(uri: str) -> Path | None:
    ...
    try:
        ...
        return None
    suffix = ".technical.xlsx" if parts[3] == "xlsx-technical" else ".xlsx"
    path = export_root(workspace_id, "xlsx") / f"{package_id}{suffix}"
...
def get_xlsx_package(
    ...
    resource_kind = str(uri).rsplit("/", 1)[-1]
    if resource_kind not in {"xlsx", "xlsx-technical"}:
        return None
    canonical_uri = PACKAGE_STORE.uri(workspace_id, package_id) + f"/{resource_kind}"
