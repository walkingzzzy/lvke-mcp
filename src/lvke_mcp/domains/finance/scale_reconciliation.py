"""Shared pre-run project-scale reconciliation.

50 公里、10 座车站的轨道线按通用单体项目种子（约 11.6 亿元）运行时，算术全部自洽，
十三表也能通过勾稽，因此 ``finance_status`` 会显示 ok —— 但业务尺度明显错误。

本模块曾只存在于零材料交付链，正式 ``finance_run_model`` 完全不做尺度对账，于是同一个
错误只在零材料链被拦住，正式链畅通无阻。城轨 Skill 明确要求"50 公里线路配通用单体项目
投资种子，即使算术自洽也必须阻断（project_scale_inconsistent），不得创建 FinanceRun"，
因此这里提升为共享门禁，两条链共用同一判定。
...
# 轨道行业判定：ProjectContext 的 industry_code 与 FinanceSpec 的 revenue.model
# 都可能标明轨道项目，任一命中即按轨道口径对账。
_RAIL_INDUSTRY_PREFIXES = (
    "urban_rail_transit",
    "rail_transit",
    "urban_rail",
    "metro",
...
def is_rail_industry(industry_code: str) -> bool:
    code = str(industry_code or "").strip().lower()
    return any(
        code == prefix or code.startswith(prefix + ".") or code.startswith(prefix + "-")
        for prefix in _RAIL_INDUSTRY_PREFIXES
    )
...
def check_finance_run_scale(
    spec: dict[str, Any] | None,
    input_revision: dict[str, Any] | None,
) -> dict[str, Any]:
    """Reconcile scale for the formal ``finance_run_model`` path.

    正式链没有 DeliveryIntent，尺度维度全部来自 spec 与 InputRevision 自身。
    行业口径优先取 spec.industry_code，其次由 ``revenue.model="rail_transit"`` 推断 ——
    后者是城轨专用收入模型，出现即说明这是轨道项目。

    线路长度与车站数只在显式提供时参与对账：它们不是通用 FinanceSpec 字段，
    缺失属于 missing_inputs 范畴，不在尺度检查内。
    """

    spec_map = dict(spec or {})
    revision = dict(input_revision or {})
    revenue = spec_map.get("revenue")
    revenue = dict(revenue) if isinstance(revenue, dict) else {}

    industry_code = str(spec_map.get("industry_code") or spec_map.get("industry") or "")
    if not is_rail_industry(industry_code) and str(revenue.get("model") or "") == "rail_transit":
        # revenue.model 已显式声明轨道口径；用它取得轨道对账，而不是因为
        # industry_code 缺失就静默跳过整条门禁。
        industry_code = "urban_rail_transit"

    def pick(*names: str) -> Any:
        for name in names:
            for holder in (revision, spec_map, revenue):
    ...
    route_km = pick("route_length_km", "line_length_km")
    if route_km is not None:
        explicit["route_length_km"] = {"value": route_km}
    station_count = pick("station_count")
    if station_count is not None:
        explicit["station_count"] = {"value": station_count}

    field_values = {
        "total_investment_wan": revision.get("total_investment_wan"),
        "build_period_months": pick("build_period_months", "build_months"),
        "operating_period_years": pick(
            "operating_period_years", "operating_years", "calc_period_years"
        ),
        "loan_ratio": pick("loan_ratio", "debt_ratio", "financing_ratio"),
        # 刻意不传 equity_ratio。债务与资本金互补是零材料假设包的不变量 —— 那里只有
        # 这两个资金来源。正式链允许第三方来源（政府补助、专项资金），例如市政道路
        # 项目 loan_ratio=0.35、capital_own_ratio=0.10，其余由 gov_subsidy_wan 补足，
        # 两者之和本就不为 1。把 capital_own_ratio 当 equity_ratio 去做互补校验会把
        # 合法资金结构判成尺度错误。正式链的资金平衡由资金筹措表与
        # non_operating_balance 负责，不在此重复。
    }
    return check_project_scale(
        industry_code=industry_code,
        explicit_inputs=explicit,
        field_values=field_values,
        # 正式链没有独立 ProjectContext 快照可比，行业一致性由上游对象链保证；
        # 这里不构造一个假的 context 去和自己比。
...
def check_project_scale(
    *,
    ...
    if is_rail_industry(industry_code) and route_km and total_investment_wan:
