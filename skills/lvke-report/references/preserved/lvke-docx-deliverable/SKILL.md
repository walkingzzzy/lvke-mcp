...
description: >
  ...
  DOCX pipeline for Lvke. Use when changing deliverable_artifacts,
  deliverable_artifact_api, professional_review basis, report-artifacts,
...
| **资产收购** | 批准 run 绑定 formal 包 | `acquisition_service` report-artifacts |
...
3. professional review status=approved 且 basis_matches=true
4. current basis fingerprint 与审查时一致
5. 财务批准 run 与工件绑定一致（域内 gate）
...
- 内部 `release`：受控发布记录，**不是**法律签章。  
- 外签：见 `lvke-backend-security-rbac`（登记/核验分离）。  
- `system_signature_performed` 必须为 false。
...
```bash
uv run pytest -q tests/test_deliverable_artifacts.py \
  tests/test_professional_review.py \
  tests/hermes_cli/test_keyui_doc_service.py
```

前端：发布页必须展示 basis 失效/未 formal 原因（`lvke-frontend-workbench`）。
...
- 跳过 basis_matches 检查  
- 把 artifact API 成功写成客户已签章  

联用：`lvke-backend-security-rbac`、`lvke-delivery-guardrails`、`lvke-finance-dual-track`。
