...
2. Call `planning_prepare_revenue_drivers` with one or more named candidates and full operating-year curves.
...
5. Call `planning_compare_revenue_candidates` and `planning_validate_revenue_drivers`.
6. Explicitly select one candidate and list every rejection in `planning_confirm_revenue_drivers`.
