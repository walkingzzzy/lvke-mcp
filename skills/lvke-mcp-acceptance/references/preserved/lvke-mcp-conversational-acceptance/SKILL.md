...
- `PASS`：真实调用并满足契约和业务预期。
- `EXPECTED_REJECTION`：真实调用后被正确的权限、签审、输入或安全门禁拒绝。
- `UPSTREAM_FAILURE`：第三方网络、权限、召回或供应商失败，本地契约正常。
...
- 外部 push、正式 release、真实签名和批准只验证真实门禁。
- 不伪造 actor、reviewer、签名、专业资质或审批。
- 宿主 tenant/actor 使用“湖北绿科”，仅表示组织宿主身份。
- “湖北绿科”不能冒充两个真实人员完成登记、核验或职责分离。
...
- 真实资料轨：真实公开资料、项目原始资料、专业签审和标准证据是否充分。
