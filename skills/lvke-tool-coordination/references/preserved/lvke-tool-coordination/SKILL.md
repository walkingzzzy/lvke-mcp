...
description: >
  ...
  multi-source search before conclusions or numeric tables: parallel
  Tavily + data_discover, then fetch/import, ingest/extract,
...
8. 宿主 tenant/actor 为“湖北绿科”仅表示组织身份，不得据此伪造 reviewer、职责分离、签名或批准。
...
| 项 | 最低要求 |
|---|---|
| 搜索通道 | ≥2 个独立 MCP/provider 并行（Tavily + data_discover provider 聚合） |
| 查询角 | ≥3 条不同 query（规模/政策/成本等） |
| 候选量 | 合并去重后尽量 ≥20；`data_discover` 宜 `target_count≥30` + `auto_expand=true` |
| 记录 | 写明各通道实际条数、不可用 provider（`data_provider_status`） |
...
**协调逻辑**:
- Tavily 与 data_discover 的 provider 聚合并行；合并 URL 去重
- 噪声大时收紧 query / `domain_allowlist`，换通道补搜，登记 issue
...
### 阶段8：统一审查与发布门禁（正式交付必经）

**目标**: 对财务十三表 / 研报做统一审查并通过 fail-closed 发布门禁；**导出 ≠ 交付**，未过审查不得对外发布。

**工具**: lvke-deliverable-review（`review_prepare / review_start / review_list_findings / review_disposition_finding / review_attest / review_retest / review_release`）

**关键约束**（与代码一致，勿放宽）:
- `report_release` / `acquisition_release_artifact` 已 **fail-closed**：`review_id` 为空或未 released 直接抛错（`professional_review_required` / `formal release requires an explicit review_id`）。导出 docx/xlsx 后若跳过审查即发布，会被 backend 拒绝。
- **P0 不可豁免**（`p0_waiver_forbidden`）；**职责分离**（`preparer_reviewer_separation_required` / `attestation_role_separation_required`）；retest 必须带证据（`retest_evidence_required`）。
- `deployment_mode=shadow` 只记录差异、**禁止 release**（`review_shadow_mode_release_forbidden`）；正式发布走 `enforced`。

**目标类型选择**（审查对象是甲方原始 xlsx 时尤其注意）:
- 结构化对象：`finance_run` / `finance_tables_package` / `report_revision` / `acquisition_run` 等——有写时租户凭证，非 local 租户可直接审查。
- 甲方上传的 xlsx：**用 `finance_xlsx_source` + `source_file_id`**（经 tenant-scoped source 存储解析），**不要**用裸路径 `finance_xlsx`——后者仅 local 兼容，非 local 租户 fail-closed（`finance_xlsx_tenant_storage_required`）。
...
review_attest(role=...)                      # 满足 ATTESTATION_ROLES 与职责分离
review_release(workspace_id, review_id)      # 通过后才可对外发布
```

章节质量改进使用 `review_list_rubrics → review_score_section → Codex propose/diff/apply → review_score_section → review_compare_assessments`。评分只生成确定性建议，不修改正文。

可复用经验使用 `lvke-knowledge-governance`：通过 rubric 且证据完整的内容才能 `knowledge_submit_candidate`；创建者与 reviewer 必须分离，只有 accepted 候选可由认证 reviewer 发布为 reviewed knowledge。禁止 Agent 直接编辑正式 Skill 或长期记忆。
...
阶段8: 统一审查与发布门禁
  ...
  review_attest(role=...)（职责分离）
  review_release(review_id)   # 通过后才是可对外交付
```
