...
上游通用示例可能写成 `./skills/self-improvement/...`；在本仓库一律用 `.claude/skills/self-improvement/`。
...
2. **Run helper** (or create manually):
   ```bash
   # hubei-lvke：默认输出到 .claude/skills/（开发 skill）
   ./.claude/skills/self-improvement/scripts/extract-skill.sh skill-name --dry-run
   ./.claude/skills/self-improvement/scripts/extract-skill.sh skill-name
   # 若脚本仍指向 ./skills，显式指定：
   ./.claude/skills/self-improvement/scripts/extract-skill.sh skill-name --output-dir .claude/skills
   ```
3. **Customize SKILL.md**: Fill in template with learning content
4. **Update learning**: Set status to `promoted_to_skill`, add `Skill-Path`（如 `.claude/skills/<name>`）
...
1. Create `.claude/skills/<skill-name>/SKILL.md`（本仓库开发 skill；**不要**默认写到仓库根 `skills/` 产品技能树）
