...
5. Run `review_prepare(target={target_type:"review_package", target_id:<confirmed package_id>})`, then `review_start`. The review must be bound to the exact package confirmed in step 2: `review_submit_assessment` compares both `target_type` and `target_id` against `review_package_id` and returns `assessment_review_package_mismatch` on any difference, and `review_finalize` returns `review_package_target_required` for any other target type. Keep `review_profile` and `review_mode` identical to the frozen package; they default to the package values when omitted, but an explicit different value is rejected as `review_profile_package_mismatch` / `review_mode_package_mismatch`. Treat deterministic findings and incomplete reasons as immutable input to semantic review.
...
10. Remediate through `propose -> diff -> apply`, import/freeze a new package, and call `review_retest`. Partial remediation is the normal case and no longer deadlocks the parent review: each finding is judged independently (`new_finding is None and coverage_ok`), so a retest closes only what was actually fixed and leaves unfixed findings open. A `standard`/`deep` child stays `retest_assessment_required` until every one of the seven dimensions has both a fresh assessment and a confirmation; a `quick` child needs only deterministic findings, never seven domains. To close a blocking finding, pass `retest_review_id` to `review_disposition_finding` alongside `closure_basis`, `before_value`, `after_value`, and precise remediation evidence — without a successful retest on the new target version the closure is rejected as `successful_retest_required`.
