...
3. 联网只使用 Tavily，通过 `data_discover` 至少执行政策、市场、成本三个 query，并用不同发布主体的来源交叉验证。Tavily 不可用时标记 `partial/upstream_failure`，不回退到其他 provider。
...
10. 技术稿可导出为 `formal_candidate`，但它仍只是技术候选工件，不表示安全签审或客户验收。
