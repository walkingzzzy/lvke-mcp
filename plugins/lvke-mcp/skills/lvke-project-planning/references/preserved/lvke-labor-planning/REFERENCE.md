...
2. 调用 `planning_infer_labor_plan`，审阅 ceil 推导轨迹；再调用 `planning_validate_labor_plan`。
3. Codex 提交确认理由，调用 `planning_confirm_labor_plan`，再以 `planning_get_labor_plan` 读取 confirmed revision。
...
- `planning_create_labor_plan` 仅为兼容入口；新正式流程不得直接填总人数绕过工作量推导。
